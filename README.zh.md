...
设置 → 插件 → **版本** 标签页，就在官方的「插件列表」标签旁边。官方那个标签只显示模块名、启用状态和 fiber 状态，不显示版本号；这个插件补上版本这一列，并额外给出安装位置、Node 版本、`DSH_HOME`、重复副本告警，以及官方包与当前 Harness 版本不一致时的提示。
...
| `src/client/locales.ts` | `en` / `zh` 词典，`en` 是键的真相来源 |
...
告警在 wire 上是**结构化的事实**，不是句子：宿主不知道读者用什么语言，所以它只报 `{ kind: 'duplicate-packages', names: [...] }`，句子由面板来写。这也让路由返回的 JSON 对其他读取方同样可用。
